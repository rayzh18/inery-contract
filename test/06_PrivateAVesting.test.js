...
contract('PrivateAVesting', (accounts) => {
  const amount = new BN('1000000000000000000000000'); // 1M * 10**18
  const price = 80000; // in USDT
  ...
  beforeEach(async function () {
    ...
    this.vesting = await PrivateAVesting.new({ from: owner });
    ...
    await this.token.transfer(this.vesting.address, totalSupply.mul(new BN(7)).div(new BN(100)), {
      from: tokenDeployer,
    });
  });
