...
contract('MarketingVesting', (accounts) => {
  ...
  beforeEach(async function () {
    ...
    this.duration = duration.months(9);

    this.vesting = await MarketingVesting.new({ from: owner });
    ...
    await this.token.transfer(this.vesting.address, totalSupply.mul(new BN(5)).div(new BN(100)), {
      from: tokenDeployer,
    });
  });
