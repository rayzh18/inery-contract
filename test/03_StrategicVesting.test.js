...
contract('StrategicVesting', (accounts) => {
  const amount = new BN('1000000000000000000000000'); // 1M * 10**18
  const price = 90000; // in USDT
  const initalAmount = amount.mul(new BN(12)).div(new BN(100)); // 12%
  ...
  beforeEach(async function () {
    ...
    this.vesting = await StrategicVesting.new({ from: owner });
    ...
    await this.token.transfer(this.vesting.address, totalSupply.mul(new BN(3)).div(new BN(100)), {
      from: tokenDeployer,
    });
  });
