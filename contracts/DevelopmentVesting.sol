...
contract DevelopmentVesting is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
	...
	/* the cliff time of the development vesting */
    uint256 private cliffTime;

	/* the duration of the development vesting */
    uint256 private duration;

    /* the address of the token contract */
    IERC20Upgradeable private tokenReward;

	/* the balances (in INERY) of all development team members */
	mapping(address => uint256) public balanceOfINERY;

	/* the whitelist of the development team */
    ...
    function initialize(
		...
		duration = 60 * 60 * 24 * 30 * 15; // 15 months (450 days)
    }
	...
	function _vestedAmount(address addr) private view returns (uint256) {
		...
		// after 1 month cliff + duration
			...
			return balanceOfINERY[addr];
		// linear part during 1 month cliff + 15 months
		} else {
